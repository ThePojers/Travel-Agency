...
class TripListOptions extends React.Component {
  handleTags(tag, checked){
    if(checked) {
      ...
      // TODO - use action dispatcher from 
      this.props.deleteTags(tag);
    }
  }
  ...
  handleDuration(type, value){
    ...
    if(type == 'to'){
      payload.to = value;
      payload.from = this.props.filters.duration.from;
    } else {
      payload.from = value;
      payload.to = this.props.filters.duration.to;
    }
    this.props.changeDuration(payload);
  }
